...
def name_key(ecosystem: str, name: str) -> tuple[str, str]:
...
def match(
    ...
    for adv in advisories:
        for pkg in adv.affected:
            index[name_key(pkg.ecosystem, pkg.name)].append((adv, pkg))

    findings: list[Finding] = []
    seen: set[tuple[str, str]] = set()
    for dep in dependencies:
        for adv, pkg in index.get(name_key(dep.ecosystem, dep.name), []):
